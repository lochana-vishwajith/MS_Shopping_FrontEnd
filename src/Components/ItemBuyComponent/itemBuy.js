...
class itemBuy extends Component {
  ...
  componentDidMount() {
    axios
      .get(
        `http://34.124.140.157:5000/itemDetails/${this.props.match.params.id}`
      )
      .then((result) => {
    ...
    axios
      .get(
        `http://localhost:5000/billingDetails/${localStorage.getItem("UserID")}`
      )
      .then((result) => {
        console.log(result.data.item);
        ...
        this.setState({
          isLoaded: true,
          BillingName: result.data.item.BillingName,
        });
  ...
  saveData = () => {
    ...
    if (this.state.detailsState == false) {
      axios
        .post(`http://localhost:5000/billingDetails/`, details)
        .then((res) => {
      ...
      axios
        .put(
          `http://localhost:5000/billingDetails/${this.state.details._id}`,
          details
        )
        .then((res) => {
      ...
      <div>
        <div className="AddressDiv">
          <div className="AddressSub1">
            <h1 className="AddText">Billing Details</h1>
            <hr className="billingHr" />
            <br />
            <TextInput
              type={"text"}
          ...
          <div className="OrderSum">
            <h1 className="AddText">Order Summary</h1>
            <hr className="billingHr" />
            <br />
            ...
            <div className="qtyDiv">
              ...
              />
              <p className="warnin">
                <small>*press space to calculate the total</small>
              </p>
            </div>
            <div className="PriceTot">
              <div>
                <h3>Price : Rs. {this.state.price}</h3>
              </div>
              <div>
                <Link to={`/delivery/${this.state.price}`}>
                  <Button
                    id={"proceed"}
                    value={"PROCEED"}
